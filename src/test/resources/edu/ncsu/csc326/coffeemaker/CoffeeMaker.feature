...
Scenario: Waiting State
   ...
   Given a default recipe book
     ...
     Scenario Outline: Testing inventory with a default recipe book
       Given a default recipe book
       When user inputs <input>
       Then An inventory of 15 coffee, 15 Milk, 15 Sugar and 15 Chocolate is displayed and the mode returns to <mode>.
     Examples:
       |input|mode|
       |1    |ADD_RECIPE|
       |2    |DELETE_RECIPE|
...
Scenario: Add a Recipe
  ...
  When user inputs 1, 50, 3,1,1,0, regular
  ...
  Scenario: Delete a Recipe
   ...
   Given a default recipe book
    When user inputs 2 1
    Then the status is OK and the mode is WAITING

    Scenario: Delete a Recipe Not IN System
    Given an empty recipe book
    When user inputs 2 1
